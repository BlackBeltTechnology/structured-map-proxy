...
=== hashCode
You cen define `hashCode` method in interface. On that case that method will be
...
You cen define `toString` method in interface. On that case that method will be
performed on the proxy's object call. If not defined the fields ara iterated and printed.
...
You cen define `equals` method in interface. On that case that method will be
