...
Options can be defined to the MapProxy's builder. For example:
...
It defines that the fields and collection in the created proxy is immutable, so all operation call cause `IllegalStateException`
- nullSafeCollection (default `false`)
If it is defined, when a collection is not set in the map an empty collection returns. Means the collection in interface never be null.
- identifierField (default `none`)
Special field which can be used to determinate equality of 2 instance. When it is not defined the String representation of the 2 objects is used for `equals`
- enumMappingMethod (default `name`)
When enum type defined as a field, have to be defined what type of representation is coded in the map. `name` menas the name of enum value is used. `getOrdinal` means the ordinal value. Any other specific method can be used whic is defined in the enum type.
